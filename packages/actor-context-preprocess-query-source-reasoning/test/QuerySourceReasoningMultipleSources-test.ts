...
import type { IClosingCondition, IRuleGraph } from '../lib';
...
describe('QuerySourceReasoningMultipleSources', () => {
  describe('constructor', () => {
    ...
    it('should get the selected shape and the reference value and the implicit quad store should remain open', async() => {
      ...
      jest.spyOn(AbstractQuerySourceReasoning, 'generateImplicitQuads').mockImplementation().mockReturnValueOnce(<any>{
      ...
      await expect(querySource.getSelectorShape()).resolves.toStrictEqual(selectorShape);
  ...
  describe('addSource', () => {
    ...
    it('should add a sources with no quads', async() => {
      ...
      await expect(implicitQuadStream.toArray()).resolves.toStrictEqual([]);
    });
